...
function Cartprod(props) {
  ...
  const updateStatus = () => {
    axios
      .delete(
        `http://localhost:8080/deleteitem?id=${props.Product_ID}&userID=${currUser}`,
        config
      )
      .then((res) => {
        console.log(res);
    ...
    <div className="parentofall">
      <div className="child-parent">
        <div className="onechild">
          <img src={props.image} className="imageprod" />
        </div>
        ...
        <div className="threechild">
          <button className="removeitem-btn" onClick={cancelHandler}>
            Remove Item
          </button>
        </div>
      </div>
      {isCancel && (
        <Cancelpopup
          content={
            <>
              <div className="alertheader">Sure?</div>
              <div className="suretocancel">
                Are you sure you want to cancel {props.product_name} from cart?
              </div>
              <br></br>
              <button className="proceed-btn" onClick={updateStatus}>
                Proceed
              </button>
            </>
          }
